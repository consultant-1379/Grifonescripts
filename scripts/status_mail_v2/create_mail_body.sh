...
if [[ $server_status == *"UNSTABLE"* ]];then
  server_status_color="#ff3232"
fi

if [[ $server_status == *"MAINTENANCE"* ]];then
  server_status_color="#ff3232"
fi
...
<h3><strong><span style="color: #ffffff;">$cluster_id $server_status</span></strong></h3>
</td></tr>
...
saveifs=$IFS
IFS="|"
for issue in $additional_status_info;do
...
ticket_team=$(echo $ticket | awk -F'|' '{print $3}')
cat <<_EOF >>$mail_body
<tr><td style="width: 498px; background-color: #ffffff; padding: 0px 50px; text-align: left;">
<p style="margin: 0px;"><a href="$ticket_url">$ticket_descr $team</a></p>
</td></tr>
_EOF
done
IFS=$saveifs
